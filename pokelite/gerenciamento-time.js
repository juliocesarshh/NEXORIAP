...
function renderizarDetalheGerenciamento(m,base){
 ...
 <p><b>Habilidade:</b> ${hab?.nome||m.habilidade||base.habilidade||"Nenhuma"}</p>
 ${['pratica','multiplayer'].includes(nexoriaContextoRevisao?.modo) && Array.isArray(base.habilidades) && base.habilidades.length ? `<label class="campo-natureza"><b>Habilidade:</b> <select id="seletor-habilidade">${base.habilidades.map(c=>`<option value="${c}" ${m.habilidade===c?"selected":""}>${typeof nomeHabilidade==='function'?nomeHabilidade(c):c}</option>`).join("")}</select></label>` : ""}
 ...
 const hs=el.querySelector("#seletor-habilidade"); if(hs) hs.onchange=()=>{m.habilidade=hs.value; nexoriaMonstroSelecionado=m.numero; renderizarGerenciamentoTime();};
