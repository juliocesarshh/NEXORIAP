...
let nexoriaTimeRevisao=[], nexoriaContextoRevisao=null;
function abrirTelaGerenciamentoTime(time,opcoes={}) {
  nexoriaTimeRevisao=time||[]; nexoriaContextoRevisao=opcoes;
...
function renderizarGerenciamentoTime(){
 ...
 nexoriaTimeRevisao.forEach((m,i)=>{
  ...
  b.onclick=()=>renderizarDetalheGerenciamento(m,DADOS_MONSTROS.find(x=>x.numero===m.numero)); lista.appendChild(b);
  if(i===0) renderizarDetalheGerenciamento(m,DADOS_MONSTROS.find(x=>x.numero===m.numero));
 });
}
function renderizarDetalheGerenciamento(m,base){
 ...
 const itens=(window.DADOS_ITENS||[]).filter(x=>x&&(!x.tipo||tipos.includes(x.tipo))).filter(x=>!["evolucao_especifica","evolucao_proxima"].includes(x.efeito?.tipo)).map(x=>`<button type="button" class="item-revisao ${m.item===x.codigo?"equipado":""}" data-item="${x.codigo}"><strong>${x.nome}</strong><small>${x.codigo} · ${x.descricao||""}</small></button>`).join("");
 ...
 const ns=el.querySelector("#seletor-natureza"); if(ns) ns.onchange=()=>{m.natureza=ns.value; if(typeof recalcularStatusDaInstancia==="function") recalcularStatusDaInstancia(m); renderizarDetalheGerenciamento(m,base); renderizarGerenciamentoTime();};
 el.querySelectorAll("[data-usar-consumivel]").forEach(b=>b.onclick=()=>{usarItemConsumivel(m,b.dataset.usarConsumivel); renderizarDetalheGerenciamento(m,base); renderizarGerenciamentoTime();});
}
